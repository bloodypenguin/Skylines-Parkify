...
namespace Parkify
{
    public static class PatchUtil
    {
            ...
            MethodDefinition transpiler = null)
        {
            ...
            {
                throw new Exception(
                    $"Parkify: prefix, postfix and transpiler are null for method {original.Type.FullName}.{original.MethodName}");
            }

            try
            {
                Debug.Log($"Parkify: Patching method {original.Type.FullName}.{original.MethodName}");
            ...
            {
                Debug.LogError($"Parkify: Failed to patch method {original.Type.FullName}.{original.MethodName}");
        ...
        {
            Debug.Log($"Parkify: Unpatching method {original.Type.FullName}.{original.MethodName}");
        ...
        private static MethodInfo GetOriginal(MethodDefinition original)
        {
            ...
            {
                throw new Exception(
                    $"Parkify: Failed to find original method {original.Type.FullName}.{original.MethodName}");
            }
        ...
        private static MethodInfo GetPatch(MethodDefinition patch)
        {
            ...
            if (methodInfo == null)
            {
                throw new Exception($"Parkify: Failed to find patch method {patch.Type.FullName}.{patch.MethodName}");
            }
